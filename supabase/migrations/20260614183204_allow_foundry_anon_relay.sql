...
grant select (id, world_id, foundry_user_id), insert (world_id, foundry_user_id, display_name)
on public.players to anon;
grant insert (world_id, player_id, formula, result, is_critical, is_fumble, roll_type, system_data)
on public.rolls to anon;
grant execute on function public.lumenn_request_world_token() to anon;

create policy "Anon can read world matching relay token"
on public.worlds
for select
to anon
using (
...
create policy "Anon can read players matching relay token"
on public.players
for select
to anon
using (
  exists (
    ...
    where w.id = players.world_id
      and w.world_token = public.lumenn_request_world_token()
  )
);

create policy "Anon can insert players matching relay token"
on public.players
for insert
to anon
with check (
...
create policy "Anon can insert rolls for matching relay token and player"
on public.rolls
for insert
to anon
with check (
